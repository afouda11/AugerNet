...
Called by: auger_cnn/train_auger_cnn_model.py
Mirrors:  augernet/gnn_train_utils.py (called by auger_pred/train_auger_model.py)
"""
...
# ---- Predefined architecture presets (for convenience) ----------------------
ARCHITECTURE_PRESETS = {
    # ── Physics-informed design (spectral analysis, Feb 2026) ─────────
    # 4-block, wide-to-narrow kernels.  RF = 43.3 eV — covers the full
    # ~40 eV discriminative region of Auger spectra.
    # Pool=3: 732 → 244 → 81 → 27 → 9 spatial.  ~600K params.
    'recommended': {
    ...
    # Legacy 3-block preset (RF = 18.8 eV — too narrow for class discrimination)
    'legacy_3block': {
...
class AugerCNN1D(nn.Module):
    ...
    Supports two modes for injecting scalar features (e.g. delta_be):
      • **early fusion** (default / legacy): scalar prepended to the 1D spectrum
        signal and processed through convolutions together.
      • **late fusion** (``late_fusion_features > 0``): the first
        ``late_fusion_features`` elements of the input are stripped off
        before convolution, then concatenated with the flattened CNN
        output and fed into the FC head.  This lets the CNN learn purely
        spectral features while the FC layers combine them with CEBE.
    ...
    late_fusion_features : int
        Number of scalar features at the start of the input vector to strip
        off and inject into the FC head (late fusion).  0 = early fusion /
        no special handling (legacy behaviour).
    """

    def __init__(self, input_length: int, num_classes: int,
                 architecture: dict, late_fusion_features: int = 0):
        super().__init__()
        ...
        self.late_fusion_features = late_fusion_features

        # Length of the spectrum that actually enters the conv blocks
        self.spectrum_length = input_length - late_fusion_features
        ...
        conv_out_len = self.spectrum_length
        for _ in range(n_blocks):
            conv_out_len = conv_out_len // pool_size
        self.flat_size = filters[-1] * conv_out_len

        # ---- build FC layers --------------------------------------------------
        # First FC input = flattened CNN output + late-fusion scalar features
        fc_input_size = self.flat_size + late_fusion_features
    ...
    def forward(self, x):
        ...
        if self.late_fusion_features > 0:
            # Strip scalar features from the front of the signal
            scalar_feats = x[:, 0, :self.late_fusion_features]       # (batch, n_scalars)
            x = x[:, :, self.late_fusion_features:]                  # (batch, 1, spectrum_length)
        ...
        if self.late_fusion_features > 0:
            # Concatenate scalar features with flattened CNN output
            x = torch.cat([x, scalar_feats], dim=1)
...
class UnifiedCNNTrainer:
    ...
    Supports optional **mixup** training (Zhang et al., 2018) for improved
    generalisation.  When ``mixup_alpha > 0``, each training mini-batch is
    replaced by a convex combination of two randomly paired samples:

        x_mix = λ·x_i + (1−λ)·x_j
        loss  = λ·CE(logits, y_i) + (1−λ)·CE(logits, y_j)

    where λ ~ Beta(alpha, alpha).  This acts as a strong regulariser and
    prevents the model from memorising training examples.
    """
    ...
    def __init__(self, model: nn.Module, device: torch.device,
        ...
        if class_weights is not None:
            ...
            print(f"  Using CrossEntropyLoss with inverse-frequency class weights{ls_str}")
        else:
            self.criterion = nn.CrossEntropyLoss(label_smoothing=label_smoothing)
            print(f"  Using CrossEntropyLoss with no class weights{ls_str}")

        if mixup_alpha > 0:
            # For mixup we also need an unreduced criterion to compute per-sample losses
            if class_weights is not None:
                self.criterion_none = nn.CrossEntropyLoss(
    ...
    def _mixup_batch(self, spectra, labels):
        """Apply mixup to a batch.  Returns mixed spectra, labels_a, labels_b, lam."""
        lam = np.random.beta(self.mixup_alpha, self.mixup_alpha)
        batch_size = spectra.size(0)
        index = torch.randperm(batch_size, device=spectra.device)
        mixed = lam * spectra + (1 - lam) * spectra[index]
    ...
    def train_epoch(self, train_loader: DataLoader) -> Tuple[float, float]:
        ...
        for batch in train_loader:
            ...
            if use_mixup:
                spectra_mix, labels_a, labels_b, lam = self._mixup_batch(spectra, labels)
                logits = self.model(spectra_mix)
                loss = (lam * self.criterion_none(logits, labels_a)
                        + (1 - lam) * self.criterion_none(logits, labels_b)).mean()
                # For accuracy tracking, count prediction matching either label
...
def evaluate_with_molecule_details(
    ...
    # AND same predicted label.  This avoids double-counting symmetry-
